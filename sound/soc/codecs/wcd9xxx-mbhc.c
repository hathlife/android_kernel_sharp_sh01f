				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		 ...
		 * removal.
		 */
#ifdef CONFIG_SH_AUDIO_DRIVER /*07-051*//* 10-079 */
		...
		if (mbhc->mbhc_cfg->detect_extn_cable &&
		    ...
		    (mbhc->hph_status && mbhc->hph_status != jack_type)) {
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*07-051*//* 10-079 */
			if (mbhc->micbias_enable && mbhc->micbias_enable_cb &&
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (plug_type == PLUG_TYPE_INVALID) {
			...
			if (!mbhc->mbhc_cfg->detect_extn_cable &&
			    ...
			    mbhc->current_plug == PLUG_TYPE_NONE) {
#ifdef CONFIG_SH_AUDIO_DRIVER /*10-108*/
				WCD9XXX_BCL_LOCK(mbhc->resmgr);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*10-108*/
				wcd9xxx_report_plug(mbhc, 1,
						    SND_JACK_HEADPHONE);
#ifdef CONFIG_SH_AUDIO_DRIVER /*10-108*/
				WCD9XXX_BCL_UNLOCK(mbhc->resmgr);
...
/* COORDINATOR Qualcomm_CPL9 MERGE start */
	#ifdef CONFIG_SH_AUDIO_DRIVER /* 07-004 */
	...
	#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 07-004 */
/* COORDINATOR Qualcomm_CPL9 MERGE end */
